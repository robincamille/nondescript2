# Finds features in given list of documents (train, test)
# Trains Gaussian NB classifier on training docs
# Returns accuracy for train, test
...
trainfile = open("features_blogtrain.csv","w")
testfile = open("features_blogtest.csv","w")

datadir = 'blogtrain' #data directory
infile = open('blogtrain.txt','r')
traindocuments = infile.readlines()
infile.close()

datadir = 'blogtest' #data directory
infile = open('blogtest.txt','r')
testdocuments = infile.readlines()
infile.close()
...
def addrandom(many,x):
    randoms = []
    numrandom = x
    while numrandom > 0:
        randoms.append(randint(0,many))
        numrandom -=1
    return randoms
    
def createfeatures(docs,outfile,datadir,randomswitch):
    i = 0

    if randomswitch == 'yes':
        #create list of documents to add random phrases to
        randoms = addrandom(len(docs),2) #2 random documents
    ...
    for doc in docs:
        ...
        if randomswitch == 'yes':
            ...
            if i in randoms:
                ...
                newfile = open("addedphrases_" + doc,"w")
                newfile.write(text)
                newfile.close()
...
createfeatures(traindocuments,trainfile,'blogtrain','no')
createfeatures(testdocuments,testfile,'blogtest','yes')
...
print "Test data:\n"
datatest = genfromtxt('features_blogtest.csv', delimiter=',')
targetstest = genfromtxt('blogtest_simple.txt', delimiter=',') #targets must be numeric
